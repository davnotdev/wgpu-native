...
pub unsafe fn map_pipeline_layout_descriptor<'a>(
    ...
    let immediate_size = extras.map_or(0, |extras| {
...
pub fn write_limits_struct(wgt_limits: wgt::Limits, limits: &mut native::WGPULimits) {
    ...
    }) = unsafe { limits.nextInChain.as_ref() }
    {
        unsafe {
            ...
            >(limits.nextInChain);
            (*native_limits).maxPushConstantSize = wgt_limits.max_immediate_size;
...
pub fn map_required_limits(
    ...
    wgt_limits.max_binding_array_elements_per_shader_stage = 999;
    wgt_limits.max_binding_array_sampler_elements_per_shader_stage = 999;
    ...
    if let Some(limits) = extras {
        if limits.maxPushConstantSize != native::WGPU_LIMIT_U32_UNDEFINED {
            wgt_limits.max_immediate_size = limits.maxPushConstantSize;
        }
...
pub fn map_feature(feature: native::WGPUFeatureName) -> Option<wgt::Features> {
    ...
    match feature {
        ...
        // wgpu-rs only features
        native::WGPUNativeFeature_PushConstants => Some(Features::IMMEDIATES),
